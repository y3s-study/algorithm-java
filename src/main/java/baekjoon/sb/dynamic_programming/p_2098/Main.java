...
public class Main {
	...
	public static void main(String[] args) {
		...
		W = new int[N+1][N+1]; // W[i][j] - i에서 j로 가기 위한 비용
		...
		shortestPath = new int[N+1][1 << N]; // shortestPath[current][visited] - 현재 위치 current에서 방문했던 도시들이 visited일 때, 부분 경로 최소값
		for(int i = 1; i <= N; i++) {
	...
	public static int getShortestPath(int current, int visited) {
		...
		for(int i = 1; i <= N; i++) {
			...
			int temp = W[current][i] + getShortestPath(i, visited + (1 << (i - 1)));
